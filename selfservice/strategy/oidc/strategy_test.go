...
func TestStrategy(t *testing.T) {
	...
	viperSetProviderConfig(
		...
		newOIDCProvider(
			t, ts, remotePublic, remoteAdmin, "claimsViaUserInfo", func(c *oidc.Configuration) {
				c.ClaimsSource = oidc.ClaimsSourceUserInfo
			},
		),
		newOIDCProvider(
			t, ts, remotePublic, remoteAdmin, "neverPKCE", func(c *oidc.Configuration) {
				c.PKCE = "never"
			},
		),
		newOIDCProvider(
			t, ts, remotePublic, remoteAdmin, "autoPKCE", func(c *oidc.Configuration) {
				c.PKCE = "auto"
			},
		),
		newOIDCProvider(
			t, ts, remotePublic, remoteAdmin, "forcePKCE", func(c *oidc.Configuration) {
				c.PKCE = "force"
			},
		),
		oidc.Configuration{
	...
	makeRequestWithCookieJar := func(
		t *testing.T, provider string, action string, fv url.Values, jar *cookiejar.Jar,
		checkRedirect testhelpers.CheckRedirectFunc,
	) (*http.Response, []byte) {
	...
	makeAPICodeFlowRequest := func(t *testing.T, provider, action string) (returnToURL *url.URL) {
		res, err := testhelpers.NewDebugClient(t).Post(
			action, "application/json", strings.NewReader(
				fmt.Sprintf(
					`{
	"method": "oidc",
	"provider": %q
	...
	exchangeCodeForToken := func(
		t *testing.T, codes sessiontokenexchange.Codes,
	) (codeResponse session.CodeExchangeResponse, err error) {
	...
	assertUIError := func(t *testing.T, res *http.Response, body []byte, reason string) {
		require.Contains(
			t, res.Request.URL.String(), uiTS.URL, "Redirect does not point to UI server. Status: %d, body: %s",
			res.StatusCode, body,
		)
	...
	assertIdentity := func(t *testing.T, res *http.Response, body []byte) {
		...
		assert.Equal(
			t, claims.traits.website, gjson.GetBytes(body, "identity.traits.website").String(), "%s",
			prettyJSON(t, body),
		)
		assert.Equal(
			t, claims.metadataPublic.picture, gjson.GetBytes(body, "identity.metadata_public.picture").String(), "%s",
			prettyJSON(t, body),
		)
	}
	...
	newLoginFlow := func(t *testing.T, requestURL string, exp time.Duration, flowType flow.Type) (req *login.Flow) {
		...
		req, _, err := reg.LoginHandler().NewLoginFlow(
	...
	newRegistrationFlow := func(
		t *testing.T, redirectTo string, exp time.Duration, flowType flow.Type,
	) *registration.Flow {
		// Use NewLoginFlow to instantiate the request but change the things we need to control a copy of it.
		req, err := reg.RegistrationHandler().NewRegistrationFlow(
			httptest.NewRecorder(),
	...
	t.Run(
		"case=should fail because provider does not exist", func(t *testing.T) {
			for k, v := range []string{
				...
				t.Run(
					fmt.Sprintf("case=%d", k), func(t *testing.T) {
						res, body := makeRequest(t, "provider-does-not-exist", v, url.Values{})
						assertSystemErrorWithReason(
							t, res, body, http.StatusNotFound, "is unknown or has not been configured",
						)
					},
				)
			}
	...
	t.Run(
		"case=should fail because the issuer is mismatching", func(t *testing.T) {
			scope = []string{"openid"}
			for k, v := range []string{
				loginAction(newBrowserLoginFlow(t, returnTS.URL, time.Minute).ID),
				registerAction(newBrowserRegistrationFlow(t, returnTS.URL, time.Minute).ID),
				...
				t.Run(
					fmt.Sprintf("case=%d", k), func(t *testing.T) {
						...
						assertSystemErrorWithReason(
							t, res, body, http.StatusInternalServerError,
	...
	t.Run(
		"case=should fail because flow does not exist", func(t *testing.T) {
	...
	t.Run(
		"case=should fail because the flow is expired", func(t *testing.T) {
			for k, v := range []uuid.UUID{
				...
				t.Run(
					fmt.Sprintf("case=%d", k), func(t *testing.T) {
						...
						assert.Contains(
							t, gjson.GetBytes(body, "ui.messages.0.text").String(), "flow expired", "%s", body,
						)
					},
				)
			}
	...
	t.Run(
		"case=should fail registration because scope was not provided", func(t *testing.T) {
			subject = "foo@bar.com"
			scope = []string{}
	...
	t.Run(
		"case=should fail because password can not handle AAL2", func(t *testing.T) {
			...
			assert.Equal(
				t, text.NewErrorValidationLoginNoStrategyFound().Text,
				gjson.GetBytes(actual, "ui.messages.0.text").String(),
			)
		},
	)

	t.Run(
		"case=should fail login because scope was not provided", func(t *testing.T) {
	...
	t.Run(
		"case=should fail registration flow because subject is not an email", func(t *testing.T) {
			...
			require.Contains(t, res.Request.URL.String(), uiTS.URL, "%s", body)
			assert.Contains(
				t, gjson.GetBytes(body, "ui.nodes.#(attributes.name==traits.subject).messages.0.text").String(),
	...
	t.Run(
		"case=cannot register multiple accounts with the same OIDC account", func(t *testing.T) {
			subject = "oidc-register-then-login@ory.sh"
			scope = []string{"openid", "offline"}

			expectTokens := func(t *testing.T, provider string, body []byte) {
				i, err := reg.PrivilegedIdentityPool().GetIdentityConfidential(
					context.Background(), uuid.FromStringOrNil(gjson.GetBytes(body, "identity.id").String()),
				)
				require.NoError(t, err)
				...
				assertx.EqualAsJSONExcept(
					...
					json.RawMessage(
						fmt.Sprintf(
							`{"providers": [{"subject":"%s","provider":"%s"}]}`, subject, provider,
						),
					),
					json.RawMessage(c),
					[]string{
						"providers.0.initial_id_token", "providers.0.initial_access_token",
						"providers.0.initial_refresh_token", "providers.0.last_id_token",
						"providers.0.last_access_token", "providers.0.last_refresh_token",
					},
				)
			}

			t.Run(
				"case=should pass registration", func(t *testing.T) {
					postRegistrationWebhook := hooktest.NewServer()
					t.Cleanup(postRegistrationWebhook.Close)
					postRegistrationWebhook.SetConfig(
						t, conf.GetProvider(ctx), config.HookStrategyKey(
							config.ViperKeySelfServiceRegistrationAfter, identity.CredentialsTypeOIDC.String(),
						),
					)
			...
			t.Run(
				"case=try another registration", func(t *testing.T) {
					transientPayload := `{"data": "registration-two"}`
					postLoginWebhook := hooktest.NewServer()
					t.Cleanup(postLoginWebhook.Close)
					postLoginWebhook.SetConfig(
						t, conf.GetProvider(ctx), config.HookStrategyKey(
							config.ViperKeySelfServiceLoginAfter, identity.CredentialsTypeOIDC.String(),
						),
					)

					returnTo := fmt.Sprintf("%s/home?query=true", returnTS.URL)
					r := newBrowserRegistrationFlow(
						t, fmt.Sprintf("%s?return_to=%s", returnTS.URL, url.QueryEscape(returnTo)), time.Minute,
					)
					action := assertFormValues(t, r.ID, "valid")
					res, body := makeRequest(t, "valid", action, url.Values{"transient_payload": {transientPayload}})
	...
	t.Run(
		"case=force PKCE", func(t *testing.T) {
			r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
			action := assertFormValues(t, r.ID, "forcePKCE")
			subject = "force-pkce@ory.sh"
			scope = []string{"openid", "offline"}
			var redirects []*http.Request
			res, body := makeRequestWithCookieJar(
				t, "forcePKCE", action, url.Values{}, nil, func(_ *http.Request, via []*http.Request) error {
					redirects = via
					return nil
				},
			)
	...
	t.Run(
		"case=force PKCE, invalid verifier", func(t *testing.T) {
			...
			res, body := makeRequestWithCookieJar(
				t, "forcePKCE", action, url.Values{}, nil, func(req *http.Request, via []*http.Request) error {
	...
	t.Run(
		"case=force PKCE, code challenge params removed from initial redirect", func(t *testing.T) {
			r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
			action := assertFormValues(t, r.ID, "forcePKCE")
			subject = "force-pkce@ory.sh"
			scope = []string{"openid", "offline"}
	...
	t.Run(
		"case=PKCE prevents authorization code injection attacks", func(t *testing.T) {
			r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
			action := assertFormValues(t, r.ID, "forcePKCE")
			subject = "attacker@ory.sh"
			scope = []string{"openid", "offline"}
			var code string
			_, err := testhelpers.NewClientWithCookieJar(
				t, nil, func(req *http.Request, via []*http.Request) error {
			...
			_, err = testhelpers.NewClientWithCookieJar(
				t, jar, func(req *http.Request, via []*http.Request) error {
			...
			res, err := testhelpers.NewClientWithCookieJar(
				t, jar, nil,
			).Get(redirectURI + "?code=" + code + "&state=" + state)
			require.NoError(t, err)
	...
	t.Run(
		"case=confused providers are detected", func(t *testing.T) {
			r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
			action := assertFormValues(t, r.ID, "valid")
			subject = "attacker@ory.sh"
			scope = []string{"openid", "offline"}
			...
			assertSystemErrorWithReason(
				t, res, body, http.StatusBadRequest, "provider mismatch between internal state and URL",
			)
		},
	)
	t.Run(
		"case=automatic PKCE", func(t *testing.T) {
			r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
			...
			var redirects []*http.Request
			res, body := makeRequestWithCookieJar(
				t, "autoPKCE", action, url.Values{}, nil, func(_ *http.Request, via []*http.Request) error {
					redirects = via
					return nil
				},
			)
	...
	t.Run(
		"case=disabled PKCE", func(t *testing.T) {
			...
			var redirects []*http.Request
			res, body := makeRequestWithCookieJar(
				t, "neverPKCE", action, url.Values{}, nil, func(_ *http.Request, via []*http.Request) error {
					redirects = via
					return nil
				},
			)
	...
	t.Run(
		"case=register and then login", func(t *testing.T) {
			...
			postRegistrationWebhook.SetConfig(
				t, conf.GetProvider(ctx),
				config.HookStrategyKey(
					config.ViperKeySelfServiceRegistrationAfter, identity.CredentialsTypeOIDC.String(),
				),
			)
			postLoginWebhook.SetConfig(
			...
			getCurrentAccessToken := func(t *testing.T, provider string, body []byte) string {
				...
				require.NoError(t, err)
				c := i.Credentials[identity.CredentialsTypeOIDC].Config
				return gjson.GetBytes(c, "providers.0.last_access_token").String()
			}
			t.Run(
				"case=should pass registration", func(t *testing.T) {
					...
					r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
					action := assertFormValues(t, r.ID, "valid")
					res, body := makeRequest(
						t, "valid", action, url.Values{
					...
					assert.Equal(
						t, "valid", gjson.GetBytes(body, "authentication_methods.0.provider").String(), "%s", body,
					)
			...
			t.Run(
				"case=should pass login", func(t *testing.T) {
			...
			t.Run(
				"case=should pass double submit", func(t *testing.T) {
					...
					makeInitialRequest := func(t *testing.T, provider, action string, fv url.Values) (
					...
					secondJar.SetCookies(
						urlx.ParseOrPanic(ts.URL), lo.Filter(
							cookies, func(item *http.Cookie, index int) bool {
			...
			t.Run(
				"case=token from login should not be the same", func(t *testing.T) {
					transientPayload := `{"data": "login"}`
	...
	t.Run(
		"case=login without registered account", func(t *testing.T) {
			postRegistrationWebhook := hooktest.NewServer()
			t.Cleanup(postRegistrationWebhook.Close)
			postLoginWebhook := hooktest.NewServer()
			t.Cleanup(postLoginWebhook.Close)
	...
	t.Run(
		"case=login with Browser+JSON", func(t *testing.T) {
			...
			t.Run(
				"case=should pass login", func(t *testing.T) {
					...
					assert.Equal(
						t, "browser_location_change_required", gjson.GetBytes(body, "error.id").String(), "%s", body,
					)
	...
	t.Run(
		"suite=API with session token exchange code", func(t *testing.T) {
			scope = []string{"openid"}
			...
			loginOrRegister := func(t *testing.T, flowID uuid.UUID, code string) {
				...
				codeResponse, err := exchangeCodeForToken(
					t, sessiontokenexchange.Codes{
						InitCode:     code,
			...
			performRegistration := func(t *testing.T) {
				f := newAPIRegistrationFlow(
					t, returnTS.URL+"?return_session_token_exchange_code=true&return_to=/app_code", 1*time.Minute,
				)
			...
			t.Run(
				"case=should use redirect_to URL on failure", func(t *testing.T) {
					ctx := context.Background()
					subject = "existing-subject-api-code-testing@ory.sh"

					i := identity.NewIdentity(config.DefaultIdentityTraitsSchemaID)
					i.SetCredentials(
						identity.CredentialsTypePassword, identity.Credentials{
	...
	t.Run(
		"case=submit id_token during registration or login", func(t *testing.T) {
			viperSetProviderConfig(
				t,
				conf,
				newOIDCProvider(t, ts, remotePublic, remoteAdmin, "valid"),
			...
			for _, tc := range []testCase{
				{
					...
					expect: func(t *testing.T, res *http.Response, body []byte) {
						require.Equal(
							t, "The provider generic does not support id_token verification",
							gjson.GetBytes(body, "error.reason").String(), "%s", body,
						)
					},
				},
				{
					...
					expect: func(t *testing.T, res *http.Response, body []byte) {
						require.Equal(
							t, "Could not verify id_token", gjson.GetBytes(body, "error.reason").String(), "%s", body,
						)
				...
				},
				{
					...
					expect: func(t *testing.T, res *http.Response, body []byte) {
						require.Equal(
							t, "The id_token claims were invalid", gjson.GetBytes(body, "error.reason").String(), "%s",
							body,
						)
					},
				},
				{
					...
					expect: func(t *testing.T, res *http.Response, body []byte) {
						require.Equal(
							t, "No nonce was included in the id_token but is required by the provider",
							gjson.GetBytes(body, "error.reason").String(), "%s", body,
						)
					},
				},
				{
					...
					expect: func(t *testing.T, res *http.Response, body []byte) {
						require.Equal(
							t, "No nonce was provided but is required by the provider",
							gjson.GetBytes(body, "error.reason").String(), "%s", body,
						)
					},
				},
				...
				},
				{
					...
					expect: func(t *testing.T, res *http.Response, body []byte) {
						require.Equal(
							t, "The supplied nonce does not match the nonce from the id_token",
							gjson.GetBytes(body, "error.reason").String(), "%s", body,
						)
					},
				},
				...
				tc := tc
				t.Run(
					fmt.Sprintf("flow=registration/case=%s", tc.name), func(t *testing.T) {
				...
				t.Run(
					fmt.Sprintf("flow=login/case=%s", tc.name), func(t *testing.T) {
						provider, token, nonce := prep(&tc)
				...
				t.Run(
					fmt.Sprintf("flow=login_without_registration/case=%s", tc.name), func(t *testing.T) {
						provider, token, nonce := prep(&tc)
						rf := newAPIRegistrationFlow(t, returnTS.URL, time.Minute)
						action := assertFormValues(t, rf.ID, "test-provider")
				...
				t.Run(
					fmt.Sprintf("flow=login_with_return_session_token_exchange_code/case=%s", tc.name),
					func(t *testing.T) {
						provider, token, nonce := prep(&tc)
						lf := newAPILoginFlow(
							t, returnTS.URL+"?return_session_token_exchange_code=true&return_to=/app_code", time.Minute,
						)
						action := assertFormValues(t, lf.ID, "test-provider")
						v := url.Values{
							"id_token":       {token},
				...
				t.Run(
					fmt.Sprintf("flow=registration_with_return_session_token_exchange_code/case=%s", tc.name),
					func(t *testing.T) {
						provider, token, nonce := prep(&tc)
						lf := newAPIRegistrationFlow(
							t, returnTS.URL+"?return_session_token_exchange_code=true&return_to=/app_code", time.Minute,
						)
						action := assertFormValues(t, lf.ID, "test-provider")
						v := url.Values{
	...
	t.Run(
		"case=login without registered account with return_to", func(t *testing.T) {
			t.Run(
				"case=should pass login", func(t *testing.T) {
			...
			t.Run(
				"case=should pass login and carry over login_challenge to registration", func(t *testing.T) {
					...
					r := newBrowserLoginFlow(
						t, fmt.Sprintf("%s?login_challenge=%s", returnTS.URL, hydra.FakeValidLoginChallenge),
						time.Minute,
					)
					action := assertFormValues(t, r.ID, "valid")
					fv := url.Values{}
	...
	t.Run(
		"case=register and register again but login", func(t *testing.T) {
			...
			t.Run(
				"case=should pass second time registration", func(t *testing.T) {
					postLoginWebhook := hooktest.NewServer()
					t.Cleanup(postLoginWebhook.Close)
					postLoginWebhook.SetConfig(
						t, conf.GetProvider(ctx), config.HookStrategyKey(
			...
			t.Run(
				"case=should pass third time registration with return to", func(t *testing.T) {
					postLoginWebhook := hooktest.NewServer()
					t.Cleanup(postLoginWebhook.Close)
					postLoginWebhook.SetConfig(
						t, conf.GetProvider(ctx), config.HookStrategyKey(
	...
	t.Run(
		"case=register, merge, and complete data", func(t *testing.T) {
			for _, tc := range []struct{ name, provider string }{
				...
				t.Run(
					fmt.Sprintf("ClaimsSource=%s", tc.name), func(t *testing.T) {
						t.Run(
							"case=should fail registration on first attempt", func(t *testing.T) {
								r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
								...
								assert.Equal(
									t, "length must be >= 2, but got 1", gjson.GetBytes(
										body, "ui.nodes.#(attributes.name==traits.name).messages.0.text",
									).String(), "%s", body,
								) // make sure the field is being echoed
								assert.Equal(
									t, "traits.name", gjson.GetBytes(
										body, "ui.nodes.#(attributes.name==traits.name).attributes.name",
									).String(), "%s", body,
								) // make sure the field is being echoed
								assert.Equal(
									t, "i", gjson.GetBytes(
										body, "ui.nodes.#(attributes.name==traits.name).attributes.value",
									).String(), "%s", body,
								) // make sure the field is being echoed
								assert.Equal(
									t, "https://www.ory.sh/kratos", gjson.GetBytes(
										body, "ui.nodes.#(attributes.name==traits.website).attributes.value",
									).String(), "%s", body,
								) // make sure the field is being echoed
							},
						)
						...
						t.Run(
							"case=should pass registration with valid data", func(t *testing.T) {
								r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
								action := assertFormValues(t, r.ID, tc.provider)
								res, body := makeRequest(
									t, tc.provider, action, url.Values{"traits.name": {"valid-name"}},
								)
								assertIdentity(t, res, body)
								assert.Equal(
									t, "https://www.ory.sh/kratos",
									gjson.GetBytes(body, "identity.traits.website").String(), "%s", body,
								)
								assert.Equal(
									t, "valid-name", gjson.GetBytes(body, "identity.traits.name").String(), "%s", body,
								)
								assert.Equal(
									t, "[\"group1\",\"group2\"]",
									gjson.GetBytes(body, "identity.traits.groups").String(), "%s", body,
								)
							},
						)
					},
	...
	for _, loginHintsEnabled := range []bool{true, false} {
		t.Run(
			"login-hints-enabled="+strconv.FormatBool(loginHintsEnabled), func(t *testing.T) {
				t.Run(
					"case=should fail to register and return fresh login flow if email is already being used by password credentials",
					func(t *testing.T) {
						subject = "email-exist-with-password-strategy-lh-" + strconv.FormatBool(loginHintsEnabled) + "@ory.sh"
						scope = []string{"openid"}
						conf.MustSet(
							ctx, config.ViperKeySelfServiceRegistrationLoginHints,
							strconv.FormatBool(loginHintsEnabled),
						)

						t.Run(
							"case=create password identity", func(t *testing.T) {
								i := identity.NewIdentity(config.DefaultIdentityTraitsSchemaID)
								i.SetCredentials(
									identity.CredentialsTypePassword, identity.Credentials{
										Identifiers: []string{subject},
						...
						t.Run(
							"case=should fail registration", func(t *testing.T) {
								r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
								action := assertFormValues(t, r.ID, "valid")
								_, body := makeRequest(t, "valid", action, url.Values{})
								snapshotx.SnapshotTJSON(
									t, body, snapshotx.ExceptPaths(
										"expires_at", "updated_at", "issued_at", "id", "created_at", "ui.action",
										findCsrfTokenPath(t, body), "request_url",
									), snapshotx.ExceptNestedKeys("newLoginUrl", "new_login_url"),
								)
							},
						)

						t.Run(
							"case=should fail registration id_first strategy enabled", func(t *testing.T) {
						...
						t.Run(
							"case=should fail login", func(t *testing.T) {
								r := newBrowserLoginFlow(t, returnTS.URL, time.Minute)
								action := assertFormValues(t, r.ID, "valid")
								_, body := makeRequest(t, "valid", action, url.Values{})
								snapshotx.SnapshotTJSON(
				...
				t.Run(
					"case=should fail to register and return fresh login flow if email is already being used by oidc credentials",
					func(t *testing.T) {
						...
						t.Run(
							"case=create oidc identity", func(t *testing.T) {
								r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
	...
	t.Run(
		"case=should redirect to default return ts when sending authenticated login flow without forced flag",
		func(t *testing.T) {
	...
	t.Run(
		"case=should reauthenticate when sending authenticated login flow with forced flag", func(t *testing.T) {
	...
	t.Run(
		"case=upstream parameters should be passed on to provider", func(t *testing.T) {
			...
			t.Run(
				"case=should pass when registering", func(t *testing.T) {
			...
			t.Run(
				"case=should pass when logging in", func(t *testing.T) {
			...
			t.Run(
				"case=should ignore invalid parameters when logging in", func(t *testing.T) {
					f := newBrowserLoginFlow(t, returnTS.URL, time.Minute)
					action := assertFormValues(t, f.ID, "valid")
			...
			t.Run(
				"case=should ignore invalid parameters when registering", func(t *testing.T) {
					f := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
					action := assertFormValues(t, f.ID, "valid")
	...
	t.Run(
		"case=verified addresses should be respected", func(t *testing.T) {
			scope = []string{"openid"}
			...
			assertVerifiedEmail := func(t *testing.T, body []byte, verified bool) {
				...
				assert.Equal(
					t, "email", gjson.GetBytes(body, "identity.verifiable_addresses.0.via").String(), "%s", body,
				)
				assert.Equal(
					t, subject, gjson.GetBytes(body, "identity.verifiable_addresses.0.value").String(), "%s", body,
				)
				assert.Equal(
					t, verified, gjson.GetBytes(body, "identity.verifiable_addresses.0.verified").Bool(), "%s", body,
				)
			}

			t.Run(
				"case=should have verified address when subject matches", func(t *testing.T) {
			...
			t.Run(
				"case=should have verified address when subject matches after normalization", func(t *testing.T) {
					subject = " Denormalized-Verified-Email@ory.sh "
					r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
					action := assertFormValues(t, r.ID, "valid")
					res, body := makeRequest(
						t, "valid", action, url.Values{"traits.subject": {"denormalized-verified-EMAIL@ory.sh"}},
					)
					subject = "denormalized-verified-EMAIL@ory.sh"
					assertIdentity(t, res, body)
					subject = "denormalized-verified-email@ory.sh"
			...
			t.Run(
				"case=should have unverified address when subject does not match", func(t *testing.T) {
					subject = "changed-verified-email@ory.sh"
					r := newBrowserRegistrationFlow(t, returnTS.URL, time.Minute)
					action := assertFormValues(t, r.ID, "valid")
					res, body := makeRequest(
						t, "valid", action, url.Values{"traits.subject": {"unverified-email@ory.sh"}},
					)
	...
	t.Run(
		"case=registration should start new login flow if duplicate credentials detected", func(t *testing.T) {
			require.NoError(t, reg.Config().Set(ctx, config.ViperKeySelfServiceRegistrationLoginHints, true))
			loginWithOIDC := func(t *testing.T, c *http.Client, flowID uuid.UUID, provider string) (
				*http.Response, []byte,
			) {
			...
			checkCredentialsLinked := func(res *http.Response, body []byte, identityID uuid.UUID, provider string) {
				assert.Contains(t, res.Request.URL.String(), returnTS.URL, "%s", body)
				assert.Equal(
					t, strings.ToLower(subject), gjson.GetBytes(body, "identity.traits.subject").String(), "%s", body,
				)
				...
				assert.Equal(
					t, provider, gjson.GetBytes(i.Credentials["oidc"].Config, "providers.0.provider").String(),
			...
			t.Run(
				"case=second login is password", func(t *testing.T) {
					...
					t.Run(
						"step=create password identity", func(t *testing.T) {
							i = identity.NewIdentity(config.DefaultIdentityTraitsSchemaID)
					...
					t.Run(
						"step=should fail login and start a new flow", func(t *testing.T) {
							res, body := loginWithOIDC(t, client, loginFlow.ID, "valid")
							assert.True(t, res.Request.URL.Query().Has("no_org_ui"))
							assertUIError(
								t, res, body,
								"You tried to sign in with \"new-login-if-email-exist-with-password-strategy@ory.sh\", but that email is already used by another account. Sign in to your account with one of the options below to add your account \"new-login-if-email-exist-with-password-strategy@ory.sh\" at \"generic\" as another way to sign in.",
							)
							assert.True(
								t, gjson.GetBytes(body, "ui.nodes.#(attributes.name==identifier)").Exists(), "%s", body,
							)
							assert.True(
								t, gjson.GetBytes(body, "ui.nodes.#(attributes.name==password)").Exists(), "%s", body,
							)
							assert.Equal(
								t, "new-login-if-email-exist-with-password-strategy@ory.sh",
								gjson.GetBytes(body, "ui.messages.#(id==1010016).context.duplicateIdentifier").String(),
							)
							...
							linkingLoginFlow.CSRFToken = gjson.GetBytes(
								body, `ui.nodes.#(attributes.name=="csrf_token").attributes.value`,
							).String()
							assert.NotEqual(
								t, loginFlow.ID.String(), linkingLoginFlow.ID, "should have started a new flow",
							)
						},
					)

					t.Run(
						"step=should fail login if existing identity identifier doesn't match", func(t *testing.T) {
							res, err := client.PostForm(
								linkingLoginFlow.UIAction, url.Values{
					...
					t.Run(
						"step=should link oidc credentials to existing identity", func(t *testing.T) {
							res, err := client.PostForm(
								linkingLoginFlow.UIAction, url.Values{
									"csrf_token": {linkingLoginFlow.CSRFToken},
									"method":     {"password"},
			...
			t.Run(
				"case=second login is OIDC", func(t *testing.T) {
					...
					t.Run(
						"step=create OIDC identity", func(t *testing.T) {
							subject = email1
					...
					t.Run(
						"step=should fail login and start a new login", func(t *testing.T) {
							...
							assertUIError(
								t, res, body,
								"You tried to sign in with \"existing-oidc-identity-1@ory.sh\", but that email is already used by another account. Sign in to your account with one of the options below to add your account \"existing-oidc-identity-1@ory.sh\" at \"generic\" as another way to sign in.",
							)
							linkingLoginFlow.ID = gjson.GetBytes(body, "id").String()
							assert.NotEqual(
								t, loginFlow.ID.String(), linkingLoginFlow.ID, "should have started a new flow",
					...
					t.Run(
						"step=should fail login if existing identity identifier doesn't match", func(t *testing.T) {
							...
							res, body := loginWithOIDC(
								t, client, uuid.Must(uuid.FromString(linkingLoginFlow.ID)), "valid",
							)
					...
					t.Run(
						"step=should link oidc credentials to existing identity", func(t *testing.T) {
							res, body := loginWithOIDC(
								t, client, uuid.Must(uuid.FromString(linkingLoginFlow.ID)), "secondProvider",
							)
	...
	t.Run(
		"method=TestPopulateSignUpMethod", func(t *testing.T) {
			conf.MustSet(ctx, config.ViperKeyPublicBaseURL, "https://foo/")

			sr, err := registration.NewFlow(
				conf, time.Minute, "nosurf", &http.Request{URL: urlx.ParseOrPanic("/")}, flow.TypeBrowser,
			)
			require.NoError(t, err)
			require.NoError(
				t,
				reg.RegistrationStrategies(context.Background()).MustStrategy(identity.CredentialsTypeOIDC).(*oidc.Strategy).PopulateRegistrationMethod(
					&http.Request{}, sr,
				),
			)
				...
				strategy.ID(): {
					...
					Config: toJson(
						identity.CredentialsOIDC{
							Providers: []identity.CredentialsOIDCProvider{
...
func TestDisabledEndpoint(t *testing.T) {
	...
	t.Run(
		"case=should not callback when oidc method is disabled", func(t *testing.T) {
	...
	t.Run(
		"case=should not auth when oidc method is disabled", func(t *testing.T) {
			c := testhelpers.NewClientWithCookies(t)

			t.Run(
				"flow=settings", func(t *testing.T) {
			...
			t.Run(
				"flow=registration", func(t *testing.T) {
...
func TestPostEndpointRedirect(t *testing.T) {
	...
	t.Run(
		"case=should redirect to GET and preserve parameters"+publicTS.URL, func(t *testing.T) {
			...
			res, err := c.PostForm(
				publicTS.URL+"/self-service/methods/oidc/callback/apple", url.Values{"state": {"foo"}, "test": {"3"}},
			)
			require.NoError(t, err)
			...
			require.NoError(t, err)
			assert.Equal(
				t, publicTS.URL+"/self-service/methods/oidc/callback/apple?state=foo&test=3", location.String(),
			)
...
func findCsrfTokenPath(t *testing.T, body []byte) string {
	...
	index := slices.IndexFunc(
		nodes, func(n gjson.Result) bool {
